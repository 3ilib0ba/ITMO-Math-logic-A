public class FullExpression {
        ...
        switch (operation) {
            case AND:
                ...
                if (current.getParent() == null) {
                    ...
                    if (current.getParent().getOperation() == Expression.Operation.OR
                            || current.getParent().getOperation() == Expression.Operation.IMPL) {
                        current.getParent().setRightBranch(expression);
                        expression.setParent(current.getParent());
                        expression.setLeftBranch(current);
                        current.setParent(expression);
                        current = current.getParent();
    ...
    public static void addSkobka(String value) {
        if (value.equals("(")) {
            if (current == null) {
                allTree = new Expression(null, null, null, null, Expression.Operation.SKOBKA, "");
                current = allTree;
                if (ParsingSystem.localNot.length() != 0) {
                    current.setnotsign(ParsingSystem.localNot);
                ...
                if (ParsingSystem.localNot.length() != 0) {
                    skobka.setnotsign(ParsingSystem.localNot);
                    ParsingSystem.localNot = "";
                }
